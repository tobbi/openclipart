...
INSERT INTO openclipart_files(id, filename, title, owner, sha1, downloads, hidden, nsfw, created) SELECT ocal_files.id, filename, upload_name, openclipart_users.id, sha1, file_num_download, not upload_published, upload_date FROM ocal_files INNER JOIN openclipart_users ON openclipart_users.user_name = ocal_files.user_name;
