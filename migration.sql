...
-- FILES [RERUN]
CREATE TABLE openclipart_files(id integer NOT NULL auto_increment, filename varchar(255), title varchar(255), seo_name varchar(255), owner integer NOT NULL, sha1 varchar(40), downloads integer, hidden boolean default 0, nsfw boolean default 0, created datetime, modifed datetime, PRIMARY KEY(id), FOREIGN KEY(owner) REFERENCES openclipart_users(id));

INSERT INTO openclipart_files(id, filename, title, owner, sha1, downloads, hidden, nsfw, created) SELECT ocal_files.id, filename, upload_name, openclipart_users.id, sha1, file_num_download, not upload_published, nsfw, upload_date FROM ocal_files INNER JOIN openclipart_users ON openclipart_users.user_name = ocal_files.user_name;
